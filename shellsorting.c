...
so first outer loop runs for every n/2 elements until it becomes zero
 ...
 tree; when mid=1, becomes the j loop runs for the value taken by e.g i=mid i,e i=1; j loop runs for 1 time; when i=2, j loop runs for 2 times i,e for locations [0] and [1]
 therfore, if i=n-1, j will run for n-1 times. the total no.of comparison=(n-1)-1  is made on the elements [j] and [j+mid]
 ...
 mid=2; i is set to mid i,e i=2; n=5; 
  ...
  j=0; now condition is checked if (v[j]>v[j+mid]) which means for every one element 
